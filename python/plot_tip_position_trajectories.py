...
def main():
    ...
    parser.add_argument("--episode", type=int,
                        ...
                        ignored.""")
    parser.add_argument("-s", "--start-episode", type=int,
                        help="""Number of the first episode when plotting a
                        range of episodes.""")
    parser.add_argument("-e", "--end-episode", type=int,
                        help="""Number of the last episode when plotting a
                        range of episodes.""")
    parser.add_argument("--plain", choices=["xy", "xz"],
    ...
    if args.episode:
        ...
        for title, ylabel, a, y in plains:
            ...
            for i in range(start, end):
                ...
                ax[a].plot(dist[:, x], dist[:, y], color=(.3, .3, .3),
                           linewidth=2)
                ax[a].plot(dist[0, x], dist[0, y], "bo", zorder=40)
                ax[a].plot(dist[-1, x], dist[-1, y], "g^", zorder=40)

            circ = patches.Circle((0, 0), 0.02, edgecolor="orange",
                                  facecolor='none', linewidth=4, zorder=50)
            ...
            ax[a].xaxis.set_label_coords(1, .5)
            ax[a].yaxis.set_label_coords(.5, 1)
