...
class FingerSpaces():
    ...
    def __init__(self, num_fingers, observations_keys,
                 observations_sizes, separate_goals):
        ...
        for i in range(len(self.observations_keys)):
            self.key_to_index[self.observations_keys[i]] = \
                slice(slice_start, slice_start + self.observations_sizes[i])
            slice_start += self.observations_sizes[i]

        self.action_bounds = {"low": np.array([-math.radians(70),
                                               -math.radians(70),
                                               -math.radians(160)]
                                              * self.num_fingers),
        ...
        self.lower_bounds['end_effector_position'] = \
            [-0.5, -0.5, 0.] * self.num_fingers
        self.upper_bounds['end_effector_position'] = \
            [0.5, 0.5, 0.5] * self.num_fingers
        ...
        self.lower_bounds['object_position'] = [-0.5, -0.5, 0.]
